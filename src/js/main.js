...
const getSliderValue = () => {
	slider.closest(".syllableCount").querySelector("span").textContent =
		slider.value + " sílaba" + (slider.value > 1 ? "s" : "");
...
Object.entries(wordSections).forEach((entry) => {
	...
	value.forEach((word) => {
		...
		originWord.innerHTML = `<span class="word" data-syllables='${JSON.stringify(
			word
		)}'>${word.join("")}</span>`;
...
const updateActions = () => {
	[destination, origin, process].forEach((section) => {
		...
		if (section.textContent.trim().length)
			sectionActions.classList.add("enabled");
	});
};
updateActions();
...
document
	...
	.addEventListener("click", clearDestination);
...
document
	...
	.addEventListener("click", (e) => randomiseWords(e));
...
const addAllWords = (e) => {
	...
	document
		...
		.forEach((container) => move(container.querySelector(".word")));
	updateActions();
};
document
	.querySelector(".btn--all")
	.addEventListener("click", (e) => addAllWords(e));
...
const shuffleWord = () => {
	...
	while (allSyllables.length < syllableCount)
		allSyllables = [...allSyllables, ...allSyllables];
...
const needsReshuffle = () => {
	...
	destination.querySelectorAll(".word").forEach((word) => {
		if (
			word.textContent.toLowerCase() ==
			process.querySelector(".result").textContent.toLowerCase()
		) {
			willNeed = true;
		}
		if (
			destination.querySelectorAll(".word").length <= 1 &&
			JSON.parse(word.dataset.syllables).length <= 1
		)
			willNeed = false;
	});
	return willNeed;
...
document
	.querySelector(".btn--remix")
	.addEventListener("click", remixDestination);
